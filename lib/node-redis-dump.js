...
RedisDump.prototype.import = function(params) {
	...
	async.waterfall([
		 ...
		 */
		function(callback) {
			var items = params.data.match(new RegExp('(SET|RPUSH|SADD|ZADD) (.+?) "(.+?)"', 'g'));
			...
			var AddRecursive = function() {
				...
				var item = items.pop(),
					chinks = item.match(new RegExp('(SET|RPUSH|SADD|ZADD) (.+?)([0-9]*) "(.+?)"', 'i')),
					command = chinks[1].toLocaleLowerCase(),
					args = (command === 'zadd') ?
						[ chinks[2], chinks[3], chinks[4], Callback ] :
						[ chinks[2], chinks[4], Callback ];
				console.log(item);
				this.getClient()[ command ].apply(this.getClient(), args);
			}.bind(this);
